...
public class UserServlet extends BaseServlet
    {
         ...
         */
        private void procList(HttpServletRequest request, HttpServletResponse response, HttpSession session,
                UserBean user) throws ServletException, IOException
            {
                if (user.getLvl() != 2)
                    {
         ...
         * @param user      ログイン中のユーザー情報
         */
        private void procNew(HttpServletRequest request, HttpServletResponse response, HttpSession session,
                UserBean user) throws ServletException, IOException
            {
                if (user.getLvl() != 2)
                    {
         ...
         */
        private void procEdit(HttpServletRequest request, HttpServletResponse response, HttpSession session,
                UserBean user, String id) throws ServletException, IOException
            {
                if (user.getLvl() != 2)
                    {
                        session.setAttribute("errMessage", MESSAGE_NO_OPERATION_PRIVILEGE);
        ...
        private void procDelete(HttpServletRequest request, HttpServletResponse response, HttpSession session,
                UserBean user) throws ServletException, IOException
            {
                ...
                if (errMessage == null)
                    {
                        getServletContext().getRequestDispatcher("/WEB-INF/user/delete_success.jsp").forward(request,
                                response);
                    } else
                    {
                        session.setAttribute("errMessage", errMessage);

                        getServletContext().getRequestDispatcher("/WEB-INF/user/delete_fail.jsp").forward(request,
                                response);
                    }
            }
        ...
        private void procUpdate(HttpServletRequest request, HttpServletResponse response, HttpSession session,
                UserBean user) throws ServletException, IOException, UnsupportedEncodingException
            {
                if (user.getLvl() != 2)
                    {
                        session.setAttribute("errMessage", MESSAGE_NO_OPERATION_PRIVILEGE);
                ...
                if (errMessage == null)
                    {
                        getServletContext().getRequestDispatcher("/WEB-INF/user/update_success.jsp").forward(request,
                                response);
                    } else
                    {
                        session.setAttribute("errMessage", errMessage);
        ...
        private void procAdd(HttpServletRequest request, HttpServletResponse response, HttpSession session,
                UserBean user) throws ServletException, IOException, UnsupportedEncodingException
            {
                ...
                if (errMessage == null)
                    {
                        session.setAttribute("userEdit", userEdit);
                        getServletContext().getRequestDispatcher("/WEB-INF/user/add_success.jsp").forward(request,
                                response);
                    } else
                    {
                        session.setAttribute("errMessage", errMessage);
                        getServletContext().getRequestDispatcher("/WEB-INF/user/add_fail.jsp").forward(request,
                                response);
                    }
            }
    }
