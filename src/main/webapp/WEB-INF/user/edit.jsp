...
<body>
	...
	<div class="main">
		<h2>既存データの編集･削除</h2>
		<form name="form1" action="UserServlet" method="post" onsubmit="return funcConfirm()">
			<table>
				<tr>
					<td>ID</td>
				...
				<tr>
					...
					<td>
						<input type="text" name="login" maxlength="15" value="<%=userEdit.getLogin()%>">
					</td>
				</tr>
				<tr>
					<td class="title">氏名</td>
					<td>
						<input type="text" name="user_name" maxlength="20" value="<%=userEdit.getName()%>">
					</td>
				</tr>
				<tr>
					...
					<td>
						<select name="user_level">
							<option value="0" <%if (userEdit.getLvl() == 0) {%> selected <%}%>>一般ユーザー
							<option value="1" <%if (userEdit.getLvl() == 1) {%> selected <%}%>>顧客管理者
							<option value="2" <%if (userEdit.getLvl() == 2) {%> selected <%}%>>システム管理者
						</select>
					</td>
				</tr>
			</table>
			<table>
				<tr>
					<td colspan="2">
						<input type="checkbox" name="password_change" onclick="funcPasswordChange()"> パスワードを変更
					</td>
				</tr>
				<tr>
					<td class="title">パスワード</td>
					<td>
						<input type="password" name="password1" maxlength="20" value="********" disabled>
					</td>
				</tr>
				<tr>
					<td class="title">パスワード（確認用）</td>
					<td>
						<input type="password" name="password2" maxlength="20" value="********" disabled>
					</td>
				</tr>
			</table>
			<p>
				<button name="state" value="update">送信</button>
				<input type="button" value="戻る" onclick="history.back()"> <input type="button" value="削除"
					onclick="funcDelete()"
				>
			</p>
		</form>
